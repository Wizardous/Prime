...
class Prime:
    ...
    def __init__(self, master):
        ...
        # private var to store the result of previous number
        self.__oldvalue = ""
        ...
        # calculate the x and y coordinates to position app window in center of screen
        x = (w//2) - (self.width//2)
        y = (h//2) - (self.height//2)
        ...
        # set window resizable option off
        ...
        # connecting the text var to a callback function to monitor the text in the text box on every key press.
        ...
        # label to display the number's type (Prime/Not Prime) or waiting for input text ('Umm...').
    ...
    # Callback method to mointor the input text widget on every character entry...
    def entry_callback(self, *args):
        # get new input value form the text variable...
        string = self.test_number.get()

        # If length exceeds 10 characters change back to 10 characters and show a worning msg...
        if len(string) > 10:
            self.test_number.set(self.__oldvalue)
            self.error_lbl['text'] = "Whoa! Hold on, input limit is 10"

        # If the input string in all digits convert it to number type and print its result in result label
        ...
        # If user types any non digit character, revert back to previous input and diaplay the old numbers result and an error msg
